// app/src/main/java/com/project/gymly/PlanFragment.java
...
public class PlanFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        // Load options (levels, goals, equipment) from Firestore
        loadPlanOptionsFromDb();
    ...
    private void loadPlanOptionsFromDb() {
        setLoading(true);

        firestore.collection("planOptions")
                .document("default")
                .get()
                .addOnSuccessListener(this::handlePlanOptionsLoaded)
                .addOnFailureListener(e -> {
                    setLoading(false);
                    Toast.makeText(
                            getContext(),
                            "Failed to load plan options: " + e.getLocalizedMessage(),
                            Toast.LENGTH_LONG
                    ).show();
                });
    }

    private void handlePlanOptionsLoaded(DocumentSnapshot doc) {
        setLoading(false);

        if (!doc.exists()) {
            Toast.makeText(getContext(), "Plan options not found in Firestore", Toast.LENGTH_LONG).show();
            return;
        }

        List<String> levels = (List<String>) doc.get("fitnessLevels");
        ...
        setupLevelSpinnerFromDb(levels);
        populateChipGroupFromList(chipGroupGoals, goals);
        populateChipGroupFromList(chipGroupEquipment, equipment);
    }

    private void setupLevelSpinnerFromDb(List<String> levelsFromDb) {
        List<String> items = new ArrayList<>();
        items.add("Select level");
        items.addAll(levelsFromDb);
    ...
    private void populateChipGroupFromList(ChipGroup group, List<String> items) {
